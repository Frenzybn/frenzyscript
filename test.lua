...
local function readKeyFromFile(filename)
    ...
    if file then
        local key = file:read("*a") -- Read the entire content
        file:close()
        return key
    else
        return nil
...
local function isValidKey(inputKey)
    local correctKey = readKeyFromFile("tsuo.bin") -- Read the key from the file
...
-- Key input box
tab:Textbox("Enter Key", "Please enter your key to access the features.", true, function(inputKey)
    if isValidKey(inputKey) then
        getgenv().AutoCDK = true
        Flux:Notification("Access Granted", "You have successfully accessed the auto-farm features.", 5)
        ...
        Flux:Notification("Access Denied", "Invalid key. Please try again.", 5)
    end
end)
